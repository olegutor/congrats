...
const UI_STRINGS = Object.freeze({
  ru: Object.freeze({
    ...
    cryptoPubkey: "Публичный ключ GPG (без стегопароля)",
    password: "Стегопароль",
    verifyStegoPresence: "Проверять наличие стего",
    verifyStegoPresenceHint: "Выключите, чтобы слабый/общий стегопароль не превращал декодер в оракул наличия",
    verifyStegoPresenceDecodeHint: "Должно совпадать с настройкой при кодировании",
    capacityFixedStego: "Без проверки: фиксированный буфер {containerBytes} байт (~{payloadBytes} байт секрета), {width}×{height}",
    capacityFixedStegoJpeg: "Без проверки: фиксированный буфер {containerBytes} байт (~{payloadBytes} байт секрета)",
    doneEncodeFixed: "Готово: fixed {containerBytes} B, файл ~{fileSizeKb} КБ, изменено {changed} пикс., α={alpha}",
    doneEncodeJpegFixed: "Готово: JPEG raw Ghost ~{fileSizeKb} КБ, fixed {containerBytes} B",
    ...
    decodePgp: "GPG по публичному ключу",
    extract: "Извлечь",
    result: "Результат",
    ...
    capacityGpgJpeg: "GPG-буфер до {payloadBytes} байт (контейнер {containerBytes}), JPEG raw ~{imageBytes} байт",
    capacityGpgJpegTooSmall: "JPEG raw-ёмкость ~{bytes} байт < минимум GPG {minBytes} байт — увеличьте размер/текстуру или используйте PNG",
    ...
    doneEncodeGpgJpeg: "Готово: буфер {bufferPercent}%, контейнер {imagePercent}% raw, файл ~{fileSizeKb} КБ, GPG {containerBytes} B",
    ...
    needCoverImage: "загрузите изображение-контейнер или вставьте из буфера",
  ...
  en: Object.freeze({
    ...
    cryptoPubkey: "GPG public key (no stego passphrase)",
    password: "Stego passphrase",
    verifyStegoPresence: "Verify stego presence",
    verifyStegoPresenceHint: "Turn off so a weak/shared stego passphrase does not make the decoder a presence oracle",
    verifyStegoPresenceDecodeHint: "Must match the setting used when encoding",
    capacityFixedStego: "No verify: fixed buffer {containerBytes} bytes (~{payloadBytes} secret bytes), {width}×{height}",
    capacityFixedStegoJpeg: "No verify: fixed buffer {containerBytes} bytes (~{payloadBytes} secret bytes)",
    doneEncodeFixed: "Done: fixed {containerBytes} B, file ~{fileSizeKb} KB, changed {changed} px, α={alpha}",
    doneEncodeJpegFixed: "Done: JPEG raw Ghost ~{fileSizeKb} KB, fixed {containerBytes} B",
    ...
    capacityGpgJpeg: "GPG buffer up to {payloadBytes} bytes (container {containerBytes}), JPEG raw ~{imageBytes} bytes",
    capacityGpgJpegTooSmall: "JPEG raw capacity ~{bytes} bytes < minimum GPG {minBytes} bytes — use a larger/more textured cover or PNG",
    ...
    doneEncodeGpgJpeg: "Done: buffer {bufferPercent}%, container {imagePercent}% raw, file ~{fileSizeKb} KB, GPG {containerBytes} B",
    ...
    needCoverImage: "upload a cover image or paste from the clipboard",
